...
if (isset($_POST['recherche'])) {
    ...
    $sql = "SELECT * FROM commandes WHERE id_client LIKE '%$search_term%' OR moyen_livraison LIKE '%$search_term%'  OR date_commande LIKE '%$search_term%'";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        echo "<h2>Résultats de recherche pour \"$search_term\" :</h2>";
        echo "<table border=1>";
        echo "<tr><th>Id</th><th>Id client</th><th>Date commande</th><th>moyen livraison</th><th>modifier</th><th>supprimer</th></tr>";
